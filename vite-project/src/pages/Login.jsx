...
const Login = () => {
  ...
  const registerSessionLog = async (user, provider) => {
    try {
      ...
      // Crear documento en colección 'session_logs'
  ...
  const saveUserToFirestore = async (user, provider) => {
    ...
    if (!snap.exists()) {
      ...
      if (!providers.includes(providerId)) {
        await updateDoc(ref, {
          ...
          authProvider: providerId, // Actualizar el provider actual
          lastLogin: serverTimestamp(),
          updatedAt: serverTimestamp(),
        });
      } else {
    ...
    // IMPORTANTE: Registrar la sesión individual
    await registerSessionLog(user, provider);
  };
  ...
  const handleExistingAccount = async (email, credential, providerName) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'usuarios', existingUser.uid), {
